...
⚠️ EVERY LEAKING LITERAL HERE IS BUILT AT RUNTIME FROM FRAGMENTS, and every one of them is
    synthetic (`.invalid` is reserved by RFC 2606; the addresses are implausible). Written out
    whole, they would make THIS FILE a finding of the very scanner it tests: the guard skips
    exactly one file, its own source, and the working-tree scan runs on every commit.
"""
...
# ⚠️ ASSEMBLED, NOT WRITTEN. See the module docstring.
