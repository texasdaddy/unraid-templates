...
def selftest(compiled: list[tuple[str, re.Pattern[str]]]) -> int:
    ...
    must_fail = [
        "deploy it to the Unraid host and check the badge",
        "SCHWAB_CALLBACK_URL=https://tape-dev.your-domain.example/v1/admin/callback",
        'DATABASE_URL=postgresql://tape:x@<internal-ip>:5432/tape',
        "allowlist = '<internal-ip>/8'",
        "peer <internal-ip> is not allowed",
        "agent at <internal-ip>:8043",
        "https://unraid-host.<tailnet-host>/",
        'Default="/mnt/POOL/appdata/tape/data"',
        "_UA = 'Research ***REDACTED***'",
        "per the owner directive 2026-06-10",
    ]
    must_pass = [
